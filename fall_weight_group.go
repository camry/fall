...
func (f *Fall) dropAdvanceWeightGroup(enableAdvance bool) []*pb.Item {
    ...
    for _, master := range f.TableWeightGroupMasters() {
        ...
        // 子集权重打乱顺序
        f.Rand().Shuffle(len(tableWeightGroupSubsets[subsetId]), func(i, j int) {
            tableWeightGroupSubsets[subsetId][i], tableWeightGroupSubsets[subsetId][j] = tableWeightGroupSubsets[subsetId][j], tableWeightGroupSubsets[subsetId][i]
        })
        // 生成基于总权重的随机数
        ...
        for _, subset := range tableWeightGroupSubsets[subsetId] {
            ...
            if randWeight > rangeWeightLast && randWeight <= rangeWeightCurrent {
                glog.Debugf("Fall MasterId %d SubsetId %d AutoId %d TotalWeight %d RandWeight %d GT %d LE %d", master.GetMasterId(), subset.GetSubsetId(), subset.GetAutoId(), subsetTotalWeight, randWeight, rangeWeightLast, rangeWeightCurrent)
                isDrop = true
            }
